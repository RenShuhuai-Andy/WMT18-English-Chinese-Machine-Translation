...
class PrimeMultiheadAttention(nn.Module):
    """Shared Projection

    See " Parallel Multi-scale Attention " for more details.
    """

    def __init__(self, embed_dim, num_heads, layer_id=0, args=None, kdim=None, vdim=None, dropout=0., bias=True,
                 add_bias_kv=False, add_zero_attn=False, cur_attn_type='es'):
        super().__init__()
            ...
            if self.attn_dynamic_cat:
                if args.dynamic_gate:
                    if args.dynamic_gate == 1:  # [2] task based para
                        self.dynamic_gate_para = nn.Parameter(
                            torch.Tensor([1.0 / len(self.k_list) for _ in self.k_list]))
                    in_ratio = 2
                else:
